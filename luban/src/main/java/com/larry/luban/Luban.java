...
import static com.larry.luban.Preconditions.checkNotNull;
...
public class Luban {
    ...
    public Luban(Context context) {
        mCacheDir = getPhotoCacheDir(context);
    }
    /**
    ...
    public Luban launch() {
        ...
        if (gear == Luban.FIRST_GEAR)
            new Thread(new Runnable() {
                @Override
                public void run() {
                    File file = firstCompress(mFile);
                    if (file == null) {
                        if (compressListener != null) compressListener.onError(new Throwable());
                    } else {
    ...
    private File saveImage(String filePath, Bitmap bitmap, long size) {
        ...
        int options = 60;
        long orig = 0;      //获取照片文件大小
        try {
            orig = getFileSize(mFile);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (orig > 4000) {
            options = 45;
        } else if (orig > 3500 && orig <= 4000) {
            options = 58;
        } else if (orig > 3000 && orig <= 3500) {
            options = 65;
        } else if (orig > 2000 && orig <= 3000) {
            options = 70;
        } else if (orig > 800 && orig <= 2000) {
            options = 85;
        } else if (orig > 350 && orig <= 800) {
            options = 90;
        } else {
            options = 95;
        }
        ...
        int siz = stream.toByteArray().length / 1024;
        if (siz > 250) {         //再次判断size大小,动态调整压缩精度
            options -= 15;
        } else {
            options -= 8;
        }
        int step = 8;
        if (stream.toByteArray().length / 1024 < 200) { //如果压缩过小就给一个大的压缩精度,而且可以避免横图压缩不清晰问题
            stream.reset();
            bitmap.compress(Bitmap.CompressFormat.JPEG, 99, stream);
        }
        while (stream.toByteArray().length / 1024 > size && options > step) {
            stream.reset();
            options -= step;
            bitmap.compress(Bitmap.CompressFormat.JPEG, options, stream);
            int s = stream.toByteArray().length / 1024;
            if (s > 250) {   //判断size大小,动态调整压缩步伐
                step = 15;
            } else {
                step = 8;
            }
        ...
        try {
            ...
            fos.close();
            stream.close();
//            if (BuildConfig.DEBUG) Log.e(TAG, filename);
            if (BuildConfig.DEBUG) Log.e(TAG, filePath);
